...
def main(data):
    ...
    if(telem.mode in {MOBILITY,BOTH}):
        ...
        if ((j1_a[2] >0.1) and (abs(j1_a[4]) < 0.5)):# staight forward
            wheels.send(9,speed)
            print(speed)
            #wheels.send(0,63)
            #wheels.send(4,63)

        elif ((j1_a[2] < -0.1) and (abs(j1_a[4]) < 0.5)):#straigt backwards
            speed = abs(speed)
            wheels.send(8,speed)
            print(speed)
            #wheels.send(1,63)
            ...
            print("Stop")
        #elif ((j1_a[2] > 0.1) and (j1_a[4] > 0.5)):#forward left
            #speed = int(round(j1_a[4] * 63))
            #wheels.send(13,speed)
            #print(speed)
            #wheels.send(9,0)
            #wheels.driveBoth(speed,0)
        #elif j1_a[2] > 0.1 and j1_a[4] < -0.5:#forward right
            #speed = int(round(j1_a[4] * 63))
            #wheels.send(10,speed)
            #print("right")
'''
    else:
